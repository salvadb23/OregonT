...
OregonH.UI.refreshStats = function () {
  ...
  document.getElementById("stat-weight").innerHTML = `${Math.ceil(
    this.caravan.weight
  )} / ${this.caravan.capacity}`;

  //update caravan position
  document.getElementById("caravan").style.left =
    (380 * this.caravan.distance) / OregonH.FINAL_DISTANCE + "px";
};
...
OregonH.UI.showShop = function (products) {
  ...
  if (!this.shopInitiated) {
    ...
    shopDiv.addEventListener("click", function (e) {
      ...
      if (target.tagName == "BUTTON") {
        ...
        let bought = OregonH.UI.buyProduct({
  ...
  for (let i = 0; i < products.length; i++) {
    product = products[i];
    prodsDiv.innerHTML += `<div class="product" data-qty=${
      product.qty
      }: data-item=${product.item} data-price=${product.price}>${product.qty} ${
      product.item
      } - $${product.price}</div>`;
  }
...
OregonH.UI.fight = function () {
  ...
  let damage = Math.ceil(
    Math.max(0, firepower * 2 * Math.random() - this.caravan.firepower)
  );
