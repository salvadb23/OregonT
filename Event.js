...
OregonH.Event.eventTypes = [
  ...
  {
    //Challenge 4: STAT-Change
    type: "STAT-CHANGE",
    notification: "positive",
    stat: "wolf",
    value: 1,
    text: "Your party befriended a wild wolf!"
  },
  {
    type: "SHOP",
  ...
  },
  {
    ...
    type: "SHOP",
    notification: "neutral",
    text: "You stumbled upon a shop",
    products: [
      { item: "Burgers", qty: 10, price: 5 },
      { item: "Knives", qty: 5, price: 30 },
      { item: "Life", qty: 1, price: 1000000 },
      { item: "Pool", qty: 90, price: 20 }
    ]
  },
  {
    type: "SHOP",
...
OregonH.Event.stateChangeEvent = function(eventData) {
  ...
  if (eventData.value + this.caravan[eventData.stat] >= 0) {
    ...
    this.ui.notify(
      eventData.text + Math.abs(eventData.value),
...
OregonH.Event.attackEvent = function(eventData) {
  const firepower = Math.round(
    (0.7 + 0.6 * Math.random()) * OregonH.ENEMY_FIREPOWER_AVG
  );
